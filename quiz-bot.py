...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not reply_markup:
        await update.message.reply_text(
            "Sorry, no quiz subjects could be loaded at the moment. Please try again later or contact the administrator."
        )
...
async def start_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data.startswith("subj|"):
        ...
        target_total = min(40, total_available) # Don't try to get more than available
        ...
        await query.edit_message_text("Sorry, an unexpected error occurred. Please try /start again.")
        return ConversationHandler.END # End cleanly
    ...
    if not questions_to_ask:
         ...
         await query.edit_message_text("Sorry, no questions could be prepared for this selection.")
         return ConversationHandler.END # End cleanly
...
async def handle_next(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int | None:
    ...
    if len(answered_in_batch) >= len(current_batch_indices):
        ...
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=f"Please answer the remaining {remaining_count} question{plural} in the current batch first!"
        )
    ...
    if isinstance(update, Update) and update.effective_message:
         try:
            ...
            if not isinstance(context.error, BadRequest) or "Message is not modified" not in str(context.error):
                 await update.effective_message.reply_text("Sorry, an unexpected error occurred. Please try /start again.")
         except Exception as e:
